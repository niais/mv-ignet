...
class Processor(IO):
        ...
        if self.arg.phase == 'train':
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.save_interval == 0) or (
                    ...
                    if self.arg.model2:
                        filename2 = 'epoch{}_model2.pt'.format(epoch + 1)
                        self.io.save_model(self.model2, filename2)
                    if self.arg.model3:
                        filename3 = 'epoch{}_model3.pt'.format(epoch + 1)
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--model_args2', action=DictAction, default=dict(), help='the arguments of model')
        parser.add_argument('--model3', default=None, help='the model will be used')
        parser.add_argument('--weights3', default=None, help='the weights for network initialization')
        parser.add_argument('--model_args3', action=DictAction, default=dict(), help='the arguments of model')
        parser.add_argument('--model4', default=None, help='the model will be used')
        parser.add_argument('--weights4', default=None, help='the weights for network initialization')
        parser.add_argument('--model_args4', action=DictAction, default=dict(), help='the arguments of model')
