...
from torchlight.io import OrderedDict


def init_seed(_):
    # Different seeds will lead to different results.
    # Sometimes you need change this seed for a better initialization. Good Luck!
    seed = 20190831
...
class Processor(IO):
        ...
        if self.arg.phase == 'train':
            ...
            if os.path.isdir(self.arg.work_dir+'/train'):
                ...
                if answer == 'y':
                    ...
                    print('Dir not removed: ', self.arg.work_dir)
    ...
    def eval_log_writer(self, epoch):
        self.val_writer.add_scalar('eval_loss', self.eval_info['eval_mean_loss'], self.global_step)
        self.val_writer.add_scalar('current_result', self.current_result, self.global_step)
        self.val_writer.add_scalar('best_result', self.best_result, self.global_step)
        self.val_writer.add_scalar('epoch', epoch, self.global_step)
    ...
    def show_eval_info(self):
        for k, v in self.eval_info.items():
            self.io.print_log('\t{}: {}'.format(k, v))
        if self.arg.pavi_log:
            self.io.log('train', self.meta_info['iter'], self.eval_info)

    def show_iter_info(self, print_flag=True):
        if self.meta_info['iter'] % self.arg.log_interval == 0:
            ...
            self.io.print_log(info, print_flag=print_flag)
    ...
    def start(self):
        self.io.print_log('Parameters:\n{}\n'.format(json.dumps(vars(self.arg), sort_keys=True, indent=4)))
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                self.train(epoch)
                self.io.print_log('Done.', print_flag=False)
    ...
    def get_parser(add_help=False):
        ...
        # model
        parser.add_argument('--sparse', type=str2bool, default=False, help='print logging or not')
